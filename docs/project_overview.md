...
### 3.2 サーバーレスなフォームハンドリングと一元管理
`functions/api/submit.ts` を用いることで、バックエンドサーバーを自前で用意することなくお問い合わせフォームを設置しています。
さらに、送信されたデータは **microCMS の APIを直接叩いて保存される** アーキテクチャになっているため、別途データベース（DBaaS）を用意することなく、お知らせ（News）もお問い合わせ（Contact）も microCMS の管理画面だけで一元管理することが可能です。
